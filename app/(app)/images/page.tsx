...
import { Image as ImageIcon, Loader2, Download, Save, AlertCircle, Wand2, Sparkles } from 'lucide-react'
...
const STYLE_PRESETS = [
  { id: 'none', label: 'Aucun', prefix: '' },
  { id: 'photo', label: 'Photoréaliste', prefix: 'Photorealistic, high quality, 8K, ' },
  { id: 'illustration', label: 'Illustration', prefix: 'Digital illustration, artistic style, ' },
  { id: 'minimal', label: 'Minimaliste', prefix: 'Minimalist design, clean lines, modern, ' },
  { id: '3d', label: '3D Render', prefix: '3D render, cinema lighting, octane render, ' },
  { id: 'watercolor', label: 'Aquarelle', prefix: 'Watercolor painting style, soft colors, artistic, ' },
]

const PROMPT_SUGGESTIONS = [
  'Logo moderne pour une startup tech',
  'Paysage futuriste avec des néons',
  'Portrait professionnel style corporate',
  'Illustration de produit sur fond blanc',
  'Bannière Instagram pour une marque de mode',
  'Icône d\'application mobile minimaliste',
]
...
function saveToMediaLibrary(image: GeneratedImage) {
  try {
    ...
    const newAsset: MediaAsset = {
      ...
      size: Math.round((image.url.length * 3) / 4),
      mimeType: 'image/png',
...
export default function ImagesPage() {
  ...
  async function handleGenerate(e: FormEvent) {
    ...
    const fullPrompt = style?.prefix ? `${style.prefix}${prompt.trim()}` : prompt.trim()
    ...
    try {
      ...
      if (data.success && data.url) {
        ...
        setError(data.error || 'Aucune image générée. Essayez un autre prompt.')
      }
    } catch (err) {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-muted text-sm mt-1">Génération d&apos;images via Gemini (Imagen 3)</p>
      </div>
      ...
      <form onSubmit={handleGenerate} className="card space-y-4">
        <div className="flex gap-3">
          ...
          >
            ...
            <span className="hidden sm:inline">{isLoading ? 'Génération...' : 'Générer'}</span>
          </button>
        </div>
      ...
      {images.length > 0 && (
        <div className="flex items-center gap-2 text-sm text-muted">
          <Sparkles className="w-4 h-4 text-lantean-blue" />
          <span>{images.length} image{images.length > 1 ? 's' : ''} générée{images.length > 1 ? 's' : ''}</span>
        </div>
      )}
      ...
      {images.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-muted">
          ...
          <p className="text-sm mt-1">Les images sont automatiquement sauvegardées dans la Médiathèque</p>
        </div>
      ) : (
