...
import { 
    ...
    Mail, Calendar as CalIcon, ChevronRight, Lock, ExternalLink, Globe, AlertTriangle, Activity, Rocket, ShieldCheck
} from 'lucide-react';
...
export const Dashboard: React.FC = () => {
  ...
  // Dynamic color for Token card based on REAL usage
  const getTokenStyle = () => {
  ...
  return (
    // Suppression de bg-background pour permettre la transparence
              ...
              <div className="flex-1 p-2 space-y-1">
                  {(connStatus === 'disconnected' || connStatus === 'error') ? (
                      <div className="h-full flex flex-col items-center justify-center text-center p-8 space-y-4">
                          <div className="p-4 bg-slate-900 border border-slate-800 rounded-2xl text-slate-500"><Lock size={32}/></div>
                          <p className="text-sm text-slate-400">En attente de configuration Google Workspace.</p>
                          <button onClick={() => window.location.hash = '#workspace'} className="text-xs font-bold text-primary hover:underline flex items-center gap-1">Connecter mon compte <ExternalLink size={12}/></button>
                      </div>
                  ) : isLoading ? (
          ...
          <div className="bg-surface border border-slate-700 rounded-3xl overflow-hidden shadow-2xl flex flex-col min-h-[400px]">
              ...
              <div className="flex-1 p-2 space-y-1">
                  {(connStatus === 'disconnected' || connStatus === 'error') ? (
                      <div className="h-full flex flex-col items-center justify-center text-center p-8 space-y-4">
                          <div className="p-4 bg-slate-900 border border-slate-800 rounded-2xl text-slate-500"><Lock size={32}/></div>
                          <p className="text-sm text-slate-400">Agenda non synchronisé.</p>
                          <button onClick={() => window.location.hash = '#workspace'} className="text-xs font-bold text-primary hover:underline flex items-center gap-1">Activer l'agenda <ExternalLink size={12}/></button>
                      </div>
                  ) : isLoading ? (
                      <div className="h-full flex items-center justify-center"><Loader2 className="animate-spin text-slate-700" size={24}/></div>
