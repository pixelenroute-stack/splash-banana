...
const SUPPORTED_MODELS = {
  TEXT_FLASH: 'gemini-2.0-flash',       // Rapide et économique
  TEXT_PRO: 'gemini-1.5-pro',           // Plus capable
  TEXT_FLASH_LITE: 'gemini-1.5-flash',  // Économique
} as const;
...
export class GeminiService {
  ...
  /**
   * Crée une instance du client Gemini (nouvelle API @google/genai)
   */
  ...
  public async testApiKey(): Promise<boolean> {
    try {
      ...
      const result = await ai.models.generateContent({
        model: SUPPORTED_MODELS.TEXT_FLASH,
...
FORMAT DE SORTIE (JSON uniquement) :
{
  ...
  "advancedTechniques": [
    ...
    try {
      ...
      // Préparer le contenu avec images
...
Format: Markdown avec émojis.`;
  ...
  /**
   * Génération d'image avec service externe (Pollinations - gratuit)
   */
  async generateImage(prompt: string, aspectRatio: string = '1:1'): Promise<string> {
    const startTime = Date.now();

    try {
      // Pour la génération d'images, utiliser Pollinations.ai (gratuit, pas de clé API)
  ...
  /**
   * Génération de vidéo (simulation avec image animée ou service externe)
   */
  async generateVideo(prompt: string, duration: string = '5s'): Promise<string> {
    const startTime = Date.now();

    try {
      // Note: Pas de service vidéo gratuit disponible
      // On simule avec une image statique
      const encodedPrompt = encodeURIComponent(prompt);

      // Pour l'instant, on retourne une image comme placeholder
      const videoUrl = `https://image.pollinations.ai/prompt/${encodedPrompt}?width=1280&height=720&nologo=true`;
      ...
      // En production, intégrer avec un vrai service vidéo (Runway, Pika, etc.)
      return videoUrl;
    } catch (e: any) {
      console.error("[Gemini] Video Generation Error:", e.message);
      throw new GeminiServiceError(
        'Génération vidéo temporairement indisponible. Utilisez le mode Production avec n8n.',
        'VIDEO_UNAVAILABLE',
        false
      );
    }
