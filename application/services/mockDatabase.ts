...
// Google Accounts - Persisté dans localStorage pour survivre aux refreshes
const loadGoogleAccounts = (): Map<string, any> => {
  ...
  try {
    ...
    console.error('Failed to load Google accounts from localStorage', e);
  }
  return new Map();
};
...
const saveGoogleAccounts = (accounts: Map<string, any>) => {
  ...
  try {
    ...
    localStorage.setItem('pixel_google_accounts', JSON.stringify(obj));
  } catch (e) {
    console.error('Failed to save Google accounts to localStorage', e);
  }
};
...
export const db = {
  ...
  saveGoogleAccount: (userId: string, acc: any) => {
    googleAccounts.set(userId, acc);
    saveGoogleAccounts(googleAccounts);
    notifyUpdate('google_account');
  },
