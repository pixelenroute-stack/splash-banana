 ...
 * Le chat utilise l'API Gemini directement via /api/chat.
 */
class N8NAgentService {
  ...
  async fetchN8nWorkflow(
    ...
    if (type === 'text_transformation' || type === 'chat') {
      try {
  ...
  async sendMessage(sessionId: string, message: string): Promise<AgentResponse> {
    try {
      ...
      return { response: `Erreur: ${err.message}`, status: 'error', agentUsed: 'System' };
    }
  }
