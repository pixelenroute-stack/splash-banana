...
import { apiRouter } from '../services/apiRouter'; // Direct call
...
export const ChatInterface: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { 
        id: '0', 
        role: 'system', 
        text: 'Agent de Production (Gemini Direct).', 
        timestamp: new Date(),
        modelUsed: 'System'
    }
  ]);
  ...
  const processMessageLogic = async (text: string): Promise<ChatMessage> => {
      ...
      let responseText = "";
      let agentUsed: string = "Gemini Pro";
      ...
      try {
        ...
        // 2. ROUTAGE STANDARD API
            ...
            agentUsed = res.provider;
        }

        // 3. Traitement post-réponse (Outils simulés via RegExp si le modèle renvoie du JSON tool call)
        if (responseText) {
