...
import { 
  User, NotionClient, Project, SystemSettings, 
...
class MockDatabase {
  ...
  private systemSettings: SystemSettings = {
    ...
    prospectionConfig: { apifyApiToken: '' },
    notionConfig: { apiKey: '', clientsDatabaseId: '', projectsDatabaseId: '' },
    billingConfig: { qontoLogin: '', qontoSecretKey: '', googleDocInvoiceTemplateId: '', googleDocQuoteTemplateId: '' },
    ...
    invoices: {},
    library: { supabaseUrl: '', supabaseAnonKey: '' },
    modules: {},
    webhooks: {}
  };
    ...
    try {
      ...
      if (stored) {
        ...
        if (data.systemSettings) {
            // Clean up webhooks from stored state if present
            const { webhooks, images, videos, chat, ...cleanSettings } = data.systemSettings;
            this.systemSettings = { ...this.systemSettings, ...cleanSettings, webhooks: webhooks || {}, images: images || { endpoint: { url: '' } }, videos: videos || { endpoint: { url: '' } }, chat: chat || { provider: 'gemini' } };
            
            // Ensure new structures exist if migrating from old DB version
            if (!this.systemSettings.newsConfig) this.systemSettings.newsConfig = { newsApiKey: '', gnewsApiKey: '', openWeatherApiKey: '' };
            if (!this.systemSettings.prospectionConfig) this.systemSettings.prospectionConfig = { apifyApiToken: '' };
            if (!this.systemSettings.notionConfig) this.systemSettings.notionConfig = { apiKey: '', clientsDatabaseId: '', projectsDatabaseId: '' };
            if (!this.systemSettings.billingConfig) this.systemSettings.billingConfig = { qontoLogin: '', qontoSecretKey: '', googleDocInvoiceTemplateId: '', googleDocQuoteTemplateId: '' };
        }
      }
    } catch (e) {
